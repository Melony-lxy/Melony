		...
		<view class="notUsed" v-show="n == 0">
			...
			<view v-for="(item, i) in peoplelist" :key="i">
				<view class="tab_Content">
					<view class="tab_img" @click="godetails(item)">
						<img :src="common.imgUrl(item.avatar_url)" alt="" />
					</view>
					<view class="tab_content">
						<p class="tab_content1">{{ item.nick }}</p>
						<!-- <p class="tab_content2">{{item}}</p> -->
					</view>
					<view class="tab_content3" @click="changetype(item)">已关注</view>
				</view>
			</view>
		</view>
		<view class="used" v-show="n == 1">
			<view v-for="(item, i) in fanlist" :key="i">
				<view class="tab_Content">
					<view class="tab_img" @click="godetails(item)">
						<img :src="common.imgUrl(item.avatar_url)" alt="" />
					</view>
...
export default {
	...
	onLoad(option) {
		let choose = option.type
		this.activeIndex = choose * 1
	},
	onShow() {
		this.showtab = false
	...
	methods: {
		godetails(item) {
			uni.navigateTo({
				url: '/packgeA/PersonalCenter/PersonalCenter?uid=' + item.id
			})
		},
		changetype(item) {
			const _this = this
