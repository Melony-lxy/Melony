...
<!-- 					<view class="information_class" v-if="people.is_class_representive">
			...
			<view class="tip" @click="navigateTo('/packgeB/scorerules/scorerules')">
...
<!-- 			<view class="withdrawal" @click="navigateTo('/packgeB/cashWithdrawal/cashWithdrawal')">
		...
		<view class="order">
			<!-- <view class="calculation" v-if="people.is_class_representive">
				<view class="CalculationBox" @click="navigateTo('/packgeB/accumulatedIncome/accumulatedIncome')">
					<view>累计收益</view>
					<view>（元）</view>
					<view class="num">{{people.accumulated_income}}</view>
				</view>
				<view class="CalculationBox" @click="navigateTo('/packgeB/pendingAmount/pendingAmount')">
					<view>待结算金额</view>
					<view>（元）</view>
					<view class="num">{{people.expect_price}}</view>
				</view>
				<view class="CalculationBox" @click="navigateTo('/packgeB/cumulativeCustomers/cumulativeCustomers')">
					<view>累计客户</view>
					<view>（人）</view>
					<view class="num">{{people.cumulative_customer}}</view>
				</view>
			...
			<view class="title">
				<view class="flex">
					...
					<view class="TitleText">积分变动记录</view>
				</view>
<!-- 				<view class="flex" @click="show=true">
			...
			<view class="OrderList" v-if="list">
				<view class="OrderDetails" v-for="(item,index) in list.data" :key="index">
					...
					<view class="OrderMoney" :style="{color:item.curr_val>item.ori_val?'red':'green'}">
						{{item.curr_val>item.ori_val?'+':'-'}}{{item.offset}}积分
					</view>
				</view>
			</view>
