<template>
	<view class="content" v-if="isShow" @click.stop="shopImg()">
		<canvas @click.stop="" :style="{ width: canvasW + 'px', height: canvasH + 'px' }" canvas-id="my-canvas" v-if="showCanva" ></canvas>
		...
		methods: {
			...
			*/
			...
			async __init() {
				...
				this.ctx.fillText(this.text, (this.canvasW/ 4.9), (
				...
				this.ctx.setFillStyle('#858585');
				this.ctx.fillText('长按保存图片或者扫描二维码', (this.canvasW / 4.3), (this.canvasW/0.74))
				//底部广告
				...
				this.ctx.drawImage(qrcodeImg.path, uni.upx2px(200), (((this.canvasW - hW) / 2) + hH + uni.upx2px(200)),
