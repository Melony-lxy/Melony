<template>
	<view class="waterfalls-flow">
		<view v-for="(item,index) in data.column" :key="index" class="waterfalls-flow-column"
			...
			style="margin-bottom: 100rpx !important;">
			<view :class="['column-value',{'column-value-show':item2.o}]" v-for="(item2,index2) in columnValue(index)"
				:key="index2" :style="[s1]" @click.stop="wapperClick(item2)" style="border-radius: 16rpx !important;background: #FFFFFF !important;
border-radius: 16rpx !important;">
				...
				<image
					...
					style="border-radius: 16rpx 16rpx 0rpx 0rpx !important;" v-if="item2[data.imageKey]"></image>
					<video style="width: 100%;border-radius: 16rpx 16rpx 0rpx 0rpx !important;" :src="common.imgUrl(item2.video_url)" :autoplay="true" :muted='true' :loop="true" :controls="true" :show-play-btn="false" :show-fullscreen-btn="false" :poster="common.imgUrl(item2.video_url + '?x-oss-process=video/snapshot,t_0,f_jpg')" v-else-if="item2.video_url"></video>
				<view class="inner" v-if="data.seat==2">
					<view class="" style="font-size: 30rpx;
					box-sizing: border-box;
					font-weight: bold;
					padding: 14rpx 26rpx 0;
color: #1a2430;overflow:hidden;  
display: -webkit-box;  
...
text-overflow: ellipsis; ">
					...
					<view class="" style="display: flex;align-items: center;box-sizing: border-box;
					padding:14rpx 26rpx ;justify-content: space-between;">
						<view class="" style="display: flex;align-items: center;" @click.stop="userClick(item2)">
							<image :src="common.imgUrl(item2.user.avatar_url)" mode="aspectFill" style="width: 18px;
							height: 18px;
							box-shadow: 0 2rpx 10rpx #00000010;border-radius: 50%;"></image>
												<view class="" style="width: 120rpx;
							text-overflow: ellipsis;
							overflow: hidden;
							white-space: nowrap;    color: #808183;
							font-size: 22rpx;margin-left: 10rpx;">
													{{item2.user.nick}}
												</view>
						</view>

						<view class="" style="display: flex;align-items: center;" @click.stop="changeLike(item2)">
							<image src="@/static/deyy/nolike.png" mode="aspectFit" style="width: 40rpx;height: 40rpx;margin-right: 10rpx;vertical-align: middle;" v-if="!item2.like_status"></image>
							
							<image src="@/static/deyy/like.png" mode="aspectFit" style="width: 40rpx;height: 40rpx;margin-right: 10rpx;vertical-align: middle;" v-else></image>
							<view class="" style="    color: #808183;
    font-size: 22rpx;" >
								{{
									item2.like_count
								}}
...
<script>
	export default {
		...
		methods: {
			...
			changeLike(item){
				let changeStatus = item.like_status ? 1 : 0
				this.$axios({
					url: this.$url[51],
					data: {
						id: item.id,
						changeStatus:item.like_status ? 1 : 0,
						status: 1,
						type:0
					}
				}).then(res => {
